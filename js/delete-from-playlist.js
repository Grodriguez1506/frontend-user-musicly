...
const playlistForm = document.getElementById("deletePlaylistForm");
...
if (token) {
  ...
  const populatePlaylists = async () => {
    try {
      let response = await fetch(
        `${API_URL}/playlist/find-by-song/${song._id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  playlistForm.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/playlist/delete-song`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-type": "application/json",
        },
        body: JSON.stringify({ playlist, song: song._id }),
      });

      if (response.status == 401) {
        const newToken = await refreshToken();
        if (newToken) {
          response = await fetch(`${API_URL}/playlist/delete-song`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-type": "application/json",
